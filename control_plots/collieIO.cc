...
// /prj_root/2671/top_write/savitsky/for_%s/final_tt13_dIbad/controlplots/final/%s/%s/Central/%s/%s/%s/%s/%s_miniTree.root", lepType.Data(), runPeriod.Data(), cutSet.Data(), lepType.Data(), tagBin.Data(), WP.Data(), looseTight.Data(), Type_CP.Data());
    for (Int_t i =0; i <Ntypes; i++) {
        TString lept;
        if (lepType.CompareTo("mujets")==0) {//mujets - 7, ejets - 6
            Type_l[i] = Type_mu[i];
        ...
        TString PATH = Form("/work/budvar-clued0/fjfi-D0/tt_leptonjets/samples/split_trees_3samples_%s_1119", lept.Data());

        TString FileName = Form("%s/%s_miniTree.root", PATH.Data(), Type_l[i].Data());
    ...
    gStyle->SetOptStat(0);
    gPad->SetLeftMargin(0.045);
    gPad->SetRightMargin(0.25);    pad2J->SetBottomMargin(0.2);
