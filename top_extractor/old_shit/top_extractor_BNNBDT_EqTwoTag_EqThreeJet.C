...
//
// To Compile
// g++ -Wall $(root-config --libs --cflags) -o top_extractor_BNNBDT_EqTwoTag_EqThreeJet top_extractor_BNNBDT_EqTwoTag_EqThreeJet.C
//
// To run to one file
...
// 9	SecondBTaggedJetPt			BNN
	...
	fprintf(dest_file,"%30.20f",SecondBTaggedJetBTagMVA);
	fprintf(dest_file,"%30.20f",SecondBTaggedJetEta);
	fprintf(dest_file,"%30.20f",SecondBTaggedJetLeptonDeltaPhi);
	fprintf(dest_file,"%30.20f",SecondBTaggedJetPt);
...
//     sprintf(destination_path_variables,"%s.var_list",argv[2]);
    ...
    top_extractor_BNNBDT_EqTwoTag_EqThreeJet();
