// To Compile
// g++ -g -O2 -Wall `root-config --cflags --libs` MatchMethodsTree.C -o  MatchMethodsTree
//
// 
...
void  CreateMethodsTree(char *source_main = "colTree.root", char *source_channel1 = "colTree.in",  char *source_channel2 = "colTree.in",  char *source_channel3 = "colTree.in", char *treeName = "col",  char *outputFileName = "colTree.root"){
  ...
  TTree *t_source = (TTree *) f_source->Get("TopologicalVariables");
  ...
  Double_t EventWeight; 			t_source->SetBranchAddress("EventWeight", &EventWeight);
  ...
  Int_t     n_events[]        =  {0, 0, 0};
  Double_t  channel_links[3];
  char     *source_channel[]  =  {source_channel1, source_channel2, source_channel3, NULL};
  char     *channel_names[]   =  {"tb", "tqb", "tbtqb",NULL};
 //std::string channel_names[] = { "tb", "tqb", "tbtqb"};
  char     *channel_types[]   =  {"tb/D", "tqb/D", "tbtqb/D"};
  Float_t   max_disc[]        =  {0, 0, 0};
  Double_t  disc;
  std::vector<Double_t> channels[3];
...
//t_output->Branch(channel_names[i].c_str(),   &s,   std::string(channel_names[i]+"/D").c_str());
  ...
  if (n_events_original != n_events[i]){
    ...
    throw std::runtime_error("Number of events in the discriminant file differs from number of EventWeights in the source file: " + std::string(source_channel[i]));
    }
  }
  ...
  for (Int_t i = 0; channel_names[i]; i++){
    ...
    for (Int_t j = 0; j<n_events_original ; j++){
        t_output -> GetEvent(j);
        hist->Fill(channel_links[i],EventWeight);
        //hist->Draw();
    }
    hist->Write();
    }
...
{
  if (argc < 7){
    std::cout << "Usage: " << argv[0] << "source_main source_channel1  source_channel2  source_channel3  methods destination_path" << std::endl;
    exit(1);
  }

   CreateMethodsTree(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6]);
