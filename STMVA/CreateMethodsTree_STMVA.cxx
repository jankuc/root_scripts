...
void  CreateMethodsTree_STMVA(const char *params, const char *inputFileName, std::vector<double> &ttA_172_vector) {//,  std::vector<double> &tqb_vector , std::vector<double> &tbtqb_vector, const char *outputFileName){
  ...
  Double_t Met;                         t_input->SetBranchAddress("Met", &Met);
...
//      std::cout << "tbtqb_vector size" << tbtqb_vector.size()   << std::endl;
      throw std::runtime_error("No. of events in the discriminant files differs from number of Weights in the input file");
    }

  // declare required variables and create output branches
  t_output->Branch("Weight",         &Weight,         "Weight/D");
  t_output->Branch("Met", &Met, "Met/D");
  ...
  char     *channel_names[]   =  {"ttA_172"};//, "tqb", "tbtqb",NULL};
  char     *channel_types[]   =  {"ttA_172/D"};//, "tqb/D", "tbtqb/D"};
...
//  std::vector<double> tbtqb_vector = STMVAEvaluate(params_tbtqb.c_str(),argv[2]);
  ...
  CreateMethodsTree_STMVA(params_tb.c_str(), argv[2], tb_vector, argv[3]);
