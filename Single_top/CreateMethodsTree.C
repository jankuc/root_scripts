...
//t_output->Branch(channel_names[i].c_str(),   &s,   std::string(channel_names[i]+"/D").c_str());
  ...
  for (Int_t i = 0; source_channel[i]; i++){
    ...
    if (n_events_original != n_events[i]){
      ...
      throw std::runtime_error("No. of events in the discriminant file differs from number of EventWeights in the source file: "+ std::string(source_channel[i]));
    }
  }
