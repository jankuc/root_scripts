...
void  CreateMethodsTree(const char *inputFileName, std::string method , const char *outputFileName){
  ...
  {
    TTree *t_output = new TTree("TopologicalVariables", "TopologicalVariables");
  ...
  TTree *t_output  = (TTree*)f_output->Get("TopologicalVariables");
  for (Int_t i = 0; i<n_channels; i++){
